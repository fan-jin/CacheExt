...
public class RPlayer {
	...
	// 0.249->0.251
	// 0.049->0.051
	private static double ACTION_REPEAT_INTERVAL_MIN = 0.499/2.0;
	private static double ACTION_REPEAT_INTERVAL_MAX = 0.501/2.0;
	...
	private void handleGameMoveMessage(RGameMoveMessage message) {
		...
		//System.out.println("Updating player " + message.getPlayerId());
	}
	...
	private void sendMoveMessage(RPlayerAvatar avatar) {
		...
		if (this.active.get() && isPublishing()) {
			...
			try {
				...
				// For supporting multiple EC2 regions...
