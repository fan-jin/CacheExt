package Mammoth.NetworkEngine.RPub.LoadBalancing;
...
import Mammoth.NetworkEngine.RPub.ControlMessages.LLAUpdateControlMessage;
import Mammoth.NetworkEngine.RPub.ControlMessages.LoadBalancerStatsControlMessage;
import Mammoth.NetworkEngine.RPub.Game.RConfig;
import Mammoth.NetworkEngine.RPub.Game.Messages.RGameUpdatePlayerInfoMessage;
import Mammoth.NetworkEngine.RPub.LoadAnalyzing.Channel;
import Mammoth.NetworkEngine.RPub.LoadBalancing.CostModel.CostAnalyzer;
import Mammoth.NetworkEngine.RPub.LoadBalancing.LoadEvaluation.AveragedLoadEvaluator;
import Mammoth.NetworkEngine.RPub.LoadBalancing.LoadEvaluation.DiscreteLoadEvaluator;
import Mammoth.NetworkEngine.RPub.LoadBalancing.LoadEvaluation.LoadEvaluator;
import Mammoth.NetworkEngine.RPub.LoadBalancing.LoadEvaluation.NewPlanEstimatedLoadEvaluator;
import Mammoth.NetworkEngine.RPub.LoadBalancing.Rebalancing.DynaWANLocationRebalancer;
import Mammoth.NetworkEngine.RPub.LoadBalancing.Rebalancing.DynaWANMessageCounter;
import Mammoth.NetworkEngine.RPub.LoadBalancing.Rebalancing.DynamothRebalancer;
import Mammoth.NetworkEngine.RPub.LoadBalancing.Rebalancing.LoadBasedRebalancer;
import Mammoth.NetworkEngine.RPub.LoadBalancing.Rebalancing.Rebalancer;
...
public class LoadBalancer {
	...
	private void printLoadByteoutOld(LoadEvaluator loadEvaluator, String identifier) {
		for (RPubClientId client: loadEvaluator.getRPubClients()) {
		...
		for (int i=0; i<RConfig.getTileCountX(); i++) {
			for (int j=0; j<RConfig.getTileCountY(); j++) {
				if (index==0) {
					...
					//plan.setMapping("tile_" + i + "_" + j + "|A", new PlanMappingImpl(new PlanId(1), "tile_" + i + "_" + j + "|A", new RPubClientId(1)));
					//plan.setMapping("tile_" + i + "_" + j + "|B", new PlanMappingImpl(new PlanId(1), "tile_" + i + "_" + j + "|B", new RPubClientId(1)));
					plan.setMapping("tile_" + i + "_" + j, new PlanMappingImpl(new PlanId(0), "tile_" + i + "_" + j, new RPubClientId(0)));
				}
			}
		}
